...
export const customerEventTitles: Partial<Record<EventType, string>> = {
  ...
  PACKAGE_DELIVERED: 'Package delivered', EVALUATION_STARTED: 'Appraisal started',
...
export function customerActivity<T extends { id: string; type: EventType; createdAt: Date | string; metadata?: unknown }>(event: T) {
  ...
  if (event.type === 'STATUS_CHANGED') {
    ...
    const labels: Record<string, string> = { CANCELLED: 'Kit cancelled', EVALUATING: 'Appraisal started', SHIPPED: 'Kit shipped', RETURNED: 'Items returned' };
