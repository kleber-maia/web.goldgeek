...
export default function AccountTopNav({ customerInitial }: AccountTopNavProps) {
  ...
  return (
    <nav className="account-top-nav">
      <div className="account-top-nav-inner">
        {/* Logo */}
        <Link href="/" className="account-top-nav-logo">
          <Image
        ...
        {/* Nav Links */}
        <div className="account-top-nav-links">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`account-top-nav-link ${isActive(link) ? "active" : ""}`}
            >
              {link.label}
            </Link>
          ))}
