...
The printable digital kit displays up to three nearby staffed FedEx stores using its shipping-address snapshot (falling back to the customer's shipping address only when there is no snapshot). The shipping service retrieves locations alongside company settings. Store name, full address, and distance appear in a three-column table that fits mobile screens and remains in the printed/downloaded packet.
...
Regression coverage: `tests/fedex-locations.test.ts` checks address forwarding, result mapping/limit, staffed filtering, empty/error fallback, and a stalled OAuth request. Browser checks used a disposable development kit: three staffed stores returned near Orlando; the table fit 375px and 1280px views; the letter with locations measured 720 × 909 CSS pixels, within the letter PDF content area; the print action completed. No real shipping labels were purchased or production records changed during verification.
