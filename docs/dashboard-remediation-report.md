...
## Follow-up: preparation UX missed by the initial audit (2026-09-09)

The initial audit did not establish that the complete customer experience was free of UX defects. Two reported misses were the persistent print reminder after using Print/Download and disconnected document/refresh controls.

Digital kit access is now recorded on the current inbound shipping label (`packetAccessedAt`). Opening Print, generating the complete PDF, or acknowledging an earlier print removes that kit's preparation warning. This records document access, not physical printing or shipment. A replacement label starts unprepared; existing downloads cannot be inferred retroactively. Documents remain accessible for reprinting, and the dashboard/detail page gives packing and staffed FedEx drop-off guidance. The label-only file sits under More download options. Automatic refresh continues without the stray global control.
